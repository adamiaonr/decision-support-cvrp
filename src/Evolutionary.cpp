...
void GenerateInitialPopulation(
    ...
    // 1) Generate the remaining population elements.
...
void RunCore(
    ...
    // 1.1) Save the best solution S_b.
    sort(
        population.begin(), 
        population.end(), 
        SortCandidates);
    ...
    // 1.2) Start counting the execution time of the algorithm
    struct timeval tm;
    gettimeofday(&tm, NULL);
    double start_time = (tm.tv_sec * 1000.0) + (tm.tv_usec/1000.0);

    // 1.3) Saving some hard constraints for the CrossOver() procedure, so that 
    // appropriate CVRP instances are generated.
    int req_num_stations = S_b->stations.size();
    int req_num_routes = S_b->routes.size();
    int req_capacity = S_b->capacity;
...
void Evolutionary::Run(
    ...
    // 1.1) Generate it in a completely random fashion.
    ...
    // value).
    CVRP * model = new CVRP(std::string(dataset));
    CWS::Run(model, CWS::SEQUENTIAL, false);
    ...
    // 2) Start the evolutionary cycle.
    RunCore(
            population,
            max_generations,
            pop_size,
...
void Evolutionary::Run(
    ...
    // 1.1) Add the CWS and SA solutions to the population.
    population.push_back(cws);
    population.push_back(sa);

    // 1.1.1) The CWS solution will be the population model for individuals.
    GenerateInitialPopulation(
            cws,
            pop_size,
            population);
